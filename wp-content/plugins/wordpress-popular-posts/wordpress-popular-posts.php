...
<!-- End Wordpress Popular Posts v<?php echo $this->version; ?> -->
		...
		*/
		...
		function format_content($string, $data = array(), $rating) {
			...
			$params = array();
			$pattern = '/\{(index|excerpt|summary|stats|title|image|thumb|rating|score|url|text_title|author|category|views|comments)\}/i';
			preg_match_all($pattern, $string, $matches);

			array_map('strtolower', $matches[0]);

			if ( in_array("{index}", $matches[0]) ) {
				$string = str_replace( "{index}", $data['index'], $string );
			}
